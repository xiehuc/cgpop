...
module caf_single_push_halo
    ...
    integer(i4), private, allocatable :: &
        Rrequest(:), &   ! MPI request objects for non-blocking receive calls
        Srequest(:)      ! MPI request objects for non-blocking send calls
    integer(i4), private, allocatable :: &
        Rstatus(:,:), &  ! MPI status objects for non-blocking receive calls
        Sstatus(:,:)     ! MPI status objects for non-blocking send calls
    ...
    ! Public subroutines:
    ! ==========================
    public :: caf_single_push_update_1d_dbl
  ...
  contains
    ...
    !<
    subroutine caf_single_push_update_1d_dbl(handle,array)
        ! !INPUT PARAMETERS:
        ...
        do i=1,nSend
            ...
            BufferRecvDbl(placeval:placeval+len-1)[dest] = &
        ...
        sync all
    end subroutine caf_single_push_update_1d_dbl
    ...
    function caf_single_push_init( &
        ...
        allocate(Sstatus(MPI_STATUS_SIZE,nSend),Rstatus(MPI_STATUS_SIZE, nRecv))
        ...
        ! ===================================
        allocate(BufferRecvDbl(maxlinear)[*])
