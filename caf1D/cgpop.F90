...
program CGpop
    ...
    !-----------------------------------------------------------------------
    !-----------------------------------------
    ! CAF multiple pull with 1D data structure
    !-----------------------------------------
    if(DO_CAF_MULTI_PULL_1D) then 
        call get_timer(timer_caf_multi_pull_solver_1D,'CAF_MULTI_PULL_1D',1, &
            nprocs)
        boundary_exchange_algorithm = ALG_CAF_MULTI_PULL_1D
        call init_solvers(RHS,PRESSI,PRESSF)
        call MPI_BARRIER(MPI_COMM_OCN, ierr)
        call timer_start(timer_caf_multi_pull_solver_1D)
        do n=1,ntrials
            PRESS=PRESSI
            call esolver(RHS,PRESS)
        enddo
        call timer_stop(timer_caf_multi_pull_solver_1D)
        !----------------------------------
        ! check the accuracy of the solver
        !----------------------------------
        call CheckAnswers('CAF_MULTI_PULL_1D',PRESSF,PRESS)
    endif
    ...
    if(DO_CAF_SINGLE_PUSH_1D) then 
        call get_timer(timer_caf_single_push_solver_1D,'CAF_SINGLE_PUSH_1D',1, &
            nprocs)
    ...
    if(DO_CAF_SINGLE_PULL_1D) then 
        call get_timer(timer_caf_single_pull_solver_1D,'CAF_SINGLE_PULL_1D',1, &
            nprocs)
    ...
    if(my_task == master_task) &
        write (*,*) 'Number of trials is: ',ntrials 
