...
module gshalo
     ...
     use caf_multi_pull_halo
     use caf_single_pull_halo
     use caf_single_push_halo
    ...
    interface GSHalo_update
        module procedure GSHalo_update_1d_int
        module procedure GSHalo_update_1d_dbl 
    end interface
  ...
  contains
    ...
    !>
    subroutine GSHalo_update_1d_dbl(handle,array)
        integer(i4), intent(in) :: handle
        real(r8), intent(inout) :: array(:)

        select case (boundary_exchange_algorithm)
            case (ALG_CAF_MULTI_PULL_1D)
                call caf_multi_pull_update_1d_dbl(handle, array)
            case (ALG_CAF_SINGLE_PULL_1D)
                call caf_single_pull_update_1d_dbl(handle, array)
            case (ALG_CAF_SINGLE_PUSH_1D)
                call caf_single_push_update_1d_dbl(handle, array)
          end select
    end subroutine GSHalo_update_1d_dbl

    !<
    !! Routine to perform the boundary exchange operation when local data
    !! is stored in a 2-dimensional array of ints.
    !!
    !! @param handle  Communication handle (generated by initialization
    !!                routine).
    !! @param array   Local data array; ghost cells are updated on output.
    ...
    subroutine GSHalo_update_1d_int(handle,array)
        ...
        select case (boundary_exchange_algorithm)
            case (ALG_CAF_MULTI_PULL_1D)
                call caf_multi_pull_update_1d_int(handle, array)
            case (ALG_CAF_SINGLE_PULL_1D)
