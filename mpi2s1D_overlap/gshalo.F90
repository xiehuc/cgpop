...
module gshalo
    ...
    public :: GSHalo_update_2d_dbl_start
    public :: GSHalo_update_2d_dbl_finish
    public :: GSHalo_update_2d_int
    public :: GSHalo_init
  ...
  contains
    ...
    !>
    subroutine GSHalo_update_2d_dbl_start(handle,array)
        ...
        enddo
    end subroutine GSHalo_update_2d_dbl_start

    !***********************************************************************
    subroutine GSHalo_update_2d_dbl_finish(handle,array)
        integer(i4), intent(in) :: handle
        real(r8), intent(inout) :: array(:)
        ...
        enddo
    end subroutine GSHalo_update_2d_dbl_finish
    ...
    subroutine GSHalo_update_2d_int(handle,array)
        ...
        do i=1,nSend
            ...
            len  = Sched%SendCnt(i)
            dest = Sched%sNeigh(i)
            if(Debug) print *,'IAM: ',my_task, ' to: ',dest, &
    ...
    !***********************************************************************
    !<
    !! Construct the message passing metadata necessary to perform the MPI
    !! based boundary exchanges necessary to update the halo region. 
    !!
    ...
    function GSHalo_init(COMM,maxlinear, nTotal,nActive,LinearGdof,LinearProc) &
      result(handle)
        ...
        if(Info) &
            print *,'IAM: ',my_task,'nNeigh: ',nRecv,' Bytes received: ', &
            8*rWords,' : ', 8*RecvCnt(1:nRecv)
