...
module solvers
    ...
    use linear, only: linearmask, ntotal, nactive, max_linear, &
        convert2dtolinear, update_halo_start, update_halo_finish, &
        convertlinearto2d, initdof
  ...
  contains
    ...
    subroutine esolver(RHS,PRESS)
        ...
        call update_halo_start(X_linear)
        call update_halo_finish(X_linear)

        !==============================
        ! the RHS
        !==============================
        call Convert2DtoLinear(B_linear,RHS)
        call update_halo_start(B_linear)
        call update_halo_finish(B_linear)
        ...
        !===============================
        call update_halo_start(Minv_linear)
        call update_halo_finish(Minv_linear)
    ...
    !***********************************************************************
    !>
    !! This subroute initializes the conjugate gradient solver with state
    ...
    !! @param mask              The land mask... Which is not used in this
    !!                          version of the solver. [REMOVE]
    !! @param solv_sum_iters    The total number of iterations. [REMOVE]
