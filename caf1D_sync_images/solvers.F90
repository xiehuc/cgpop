...
module solvers
  ! !USES:
    ...
    use constants, only: c0, c1, field_type_scalar, field_loc_center, p25, &
        eps, blank_fmt, delim_fmt, boundary_exchange_algorithm, ALG_MPI2S_1D, &
        ALG_MPI2S_2D, solv_max_iters, ALG_CAF_MULTI_PULL_1D, &
        ALG_CAF_SINGLE_PULL_1D,ALG_CAF_SINGLE_PUSH_1D, ALG_CAF_SINGLE_PUSH_2D, &
        ALG_CAF_SINGLE_PULL_2D
  ...
  contains
    ...
    subroutine esolver(RHS,PRESS)
        ...
        if(solv_linear) then 
            if(my_task == master_task) &
                print *,'esolver: using the linear datastructure version'
    ...
    subroutine init_solvers(RHS,PRESSI,PRESSF)
        ...
        !-------------------------------------------------------------------
        !   resid_norm =
        !       c1/global_sum(WORK0_read, distrb_tropic, field_loc_center, &
        !           RCALCT_B)
        !-----------------------------------------------------------------------
        !
