...
!>
!! Conversion routines to and from linear datastructure.
!<
module linear
    use kinds_mod, only: i4, r8, r4
    use simple_type, only: distrb
    use communicate, only: MPI_COMM_OCN, my_task, get_num_procs, master_task
    use constants, only: c0, field_type_scalar, field_loc_center, &
        field_loc_necorner, field_loc_nface
    use simple_blocks, only: nx_block,ny_block, get_proc_id, &
        set_block_parameter, nblocks_y, get_block_parameter, &
        Neigh,iwest,ieast,isouth,inorth,all_blocks, iswest,iseast,inwest,ineast
    ...
    use domain_size, only: max_blocks_tropic, block_size_x, block_size_y, &
        nx_global
    use gshalo, only: gshalo_init, gshalo_update_1d_int, &
        gshalo_update_1d_dbl_start, gshalo_update_1d_dbl_finish
    ...
    public :: initDOF
    public :: update_halo_start
    public :: update_halo_finish
  ...
  contains
    ...
    !***********************************************************************
    !>
    !! Convert a 2D data structure into a linear array with the land points
    !! removed.
    !!
    !! @param dest   The destination linear array
    !! @param src    The 2D source array 
    !<
    subroutine Convert2DtoLinear(dest,src)
        ...
        ! !OUTPUT PARAMETERS:
        real(r8), intent(inout), dimension(max_linear) :: dest
    ...
    subroutine initDOF(fname,strip,MASK)
        ...
        if(CheckGSHalo)  then 
            ...
            call GSHalo_update_1d_int(gsHalo_handle,LinearGdof2)

            LinearDiff(nActive+1:nTotal) = LinearGdof(nActive+1:nTotal) - &
                LinearGdof2(nActive+1:nTotal)
    ...
    subroutine FormHaloDofCk(i,j,iblock,pnum, gdof,ldof,linGdof,linProc,il)
        ...
        integer(i4) :: idx, pnum, ierr
        logical :: found

        if(pnum .ne. my_task) then
            if(gdof(i,j,iblock) .gt. 0) then
                call LinearFind(linGdof(nActive+1:il),gdof(i,j,iblock),found, &
                    idx)
                if(found) then 
    ...
    !>
    subroutine update_halo_start(array)
        real(r8), dimension(:), intent(inout) :: array

        call GSHalo_update_1d_dbl_start(gshalo_handle,array)
    end subroutine update_halo_start
    ...
    !! @param array  The variable whose halo should be updated.
    !>
    subroutine update_halo_finish(array)
        real(r8), dimension(:), intent(inout) :: array

        call GSHalo_update_1d_dbl_finish(gshalo_handle,array)
    end subroutine update_halo_finish
end module linear
