...
program CGpop
    ...
    use timers, only: get_timer,timer_start,timer_stop,timer_print_all, &
        init_timers
    use caf_push_boundary, only: &
    timer_caf_boundary_create, timer_caf_boundary_2d_dbl
    ...
    !-----------------------------------------
    ! CAF single push with 2D data structure
    !-----------------------------------------
    if(DO_CAF_SINGLE_PUSH_2D) then 
        call get_timer( &
            timer_caf_single_push_solver_2D,'CAF_SINGLE_PUSH_2D',1,nprocs)

        boundary_exchange_algorithm = ALG_CAF_SINGLE_PUSH_2D
        call init_solvers(RHS,PRESSI,PRESSF)
        call MPI_BARRIER(MPI_COMM_OCN, ierr)
        call timer_start(timer_caf_single_push_solver_2D)
        do n=1,ntrials
            PRESS=PRESSI
            call esolver(RHS,PRESS)
        enddo
        call timer_stop(timer_caf_single_push_solver_2D)
        !----------------------------------
        ! check the accuracy of the solver
        !----------------------------------
        call CheckAnswers('CAF_SINGLE_PUSH_2D',PRESSF,PRESS)
    endif

    !-----------------------------------------
    ! CAF single pull  with 2D data structure
    !-----------------------------------------
    if(DO_CAF_SINGLE_PULL_2D) then 
        call get_timer( &
            timer_caf_single_pull_solver_2D,'CAF_SINGLE_PULL_2D',1,nprocs)

        boundary_exchange_algorithm = ALG_CAF_SINGLE_PULL_2D
        call init_solvers(RHS,PRESSI,PRESSF)
        call MPI_BARRIER(MPI_COMM_OCN, ierr)
        call timer_start(timer_caf_single_pull_solver_2D)
        do n=1,ntrials
            PRESS=PRESSI
            call esolver(RHS,PRESS)
        enddo
        call timer_stop(timer_caf_single_pull_solver_2D)
        !----------------------------------
        ! check the accuracy of the solver
        !----------------------------------
        call CheckAnswers('CAF_SINGLE_PULL_2D',PRESSF,PRESS)
    endif
